...
lstm_cells = [
    tf.compat.v1.estimator.rnn.LSTMCell(num_units=num_nodes[li],
                            state_is_tuple=True,
...
drop_multi_cell = tf.compat.v1.estimator.rnn.MultiRNNCell(drop_lstm_cells)
multi_cell = tf.compat.v1.estimator.rnn.MultiRNNCell(lstm_cells)

w = tf.get_variable('w',shape=[num_nodes[-1], 1], initializer=tf.compat.v1.estimator.layers.xavier_initializer())
